...
public:
	...
	const Vec2 operator+( float other ) const;                  // vec2 + vec2
	...
	const Vec2 operator/(const Vec2& inverseScale ) const;      // vec2 / float

	// Operators (self-mutating / non-const)
	void       operator+=( const Vec2& vecToAdd );              // vec2 += vec2
	void       operator+=( float other );                       // vec2 += vec2
	...
	void       operator/=(const Vec2& vecDivisor );             // vec2 /= float
